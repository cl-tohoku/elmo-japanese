...
from data import LMDataset, BidirectionalLMDataset, load_vocab
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for batch_no, batch in enumerate(data_gen, start=1):
            ...
            for k in range(n_gpus):
                ...
                feed_dict.update(
                    _get_feed_dict_from_X(X, start, end, model, char_inputs, bidirectional)
                )
...
def get_elmo_emb(args, options, ckpt_file, data):
    """
    Get elmo embeddings
    """
    ...
    with tf.Session(config=config) as sess:
        ...
        sent_index = 0
        prev_elmo = []
        outfh = h5py.File(args.output_fn, 'w')
        for batch_no, batch in enumerate(data.iter_batches(batch_size, unroll_steps), start=1):
            # slice the input in the batch for the feed_dict
            ...
            if args.print_token:
                for sent in X['token_ids']:
                    line = [data.vocab.id_to_word(w) for w in sent]
                    line = " ".join(line)
                    print(line)
            ...
            # Make & save elmo #
            ####################
            new_elmo = make_elmo(elmo_embs, elmo_lstm, unroll_steps)
            elmo_list, prev_elmo = concat_elmo(prev_elmo, new_elmo, X)
            ...
            # Print elmo shape #
            ####################
            if args.print_elmo_shape:
                print('ELMo word embs:\n'
                      '\t(1D=2(forward/backward), 2D=batch_size, '
                      '3D=unroll_steps, 4D=dim)\n'
                      '\t(%d, %d, %d, %d)' % (len(elmo_embs),
                                              elmo_embs[0].shape[0],
                                              elmo_embs[0].shape[1],
                                              elmo_embs[0].shape[2])
                      )
                print('ELMo LSTM embs:\n'
                      '\t(1D=2(forward/backward), 2D=unroll_steps, '
                      '3D=2(lstm layers), 4D=batch_size, 5D=dim)\n'
                      '\t(%d, %d, %d, %d %d)' % (len(elmo_lstm),
                                                 len(elmo_lstm[0]),
                                                 len(elmo_lstm[0][0]),
                                                 elmo_lstm[0][0][0].shape[0],
                                                 elmo_lstm[0][0][0].shape[1])
                      )
...
def make_elmo(elmo_embs, elmo_lstm, unroll_steps):
...
def concat_elmo(prev_elmo, new_elmo, X):
...
def main(args):
    ...
    get_elmo_emb(args, options, ckpt_file, data)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Compute test perplexity')
    parser.add_argument('--save_dir', help='Location of checkpoint files')
    parser.add_argument('--word_file', help='Vocabulary file')
    parser.add_argument('--char_file', help='Vocabulary file')
    parser.add_argument('--test_prefix', help='Prefix for test files')
    parser.add_argument('--batch_size', type=int, default=256, help='Batch size')
    parser.add_argument('--print_token', action='store_true', default=False)
    parser.add_argument('--print_elmo_shape', action='store_true', default=False)
    parser.add_argument('--output_fn', default='elmo.hdf5', help='The output file name')
